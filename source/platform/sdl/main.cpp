...
struct CompositeVideoDevice : public nba::VideoDevice {
    std::vector<std::unique_ptr<nba::VideoDevice>> devices;
    CompositeVideoDevice() {
        devices.emplace_back(std::make_unique<SDL2_VideoDevice>());
        devices.emplace_back(std::make_unique<SDL2_VK_VideoDevice>(*g_config));
    }
...
void usage(char* app_name) {
    fmt::print("Usage: {0} [--bios bios_path] [--force-rtc] [--save-type type] [--fullscreen] "
               "[--scale factor] [--resampler type] [--sync-to-audio yes/no] rom_path\n",
               app_name);
    std::exit(-1);
}
...
void load_keymap() {
    ...
    if (data.contains("general")) {
        ...
        if (general_result.is_ok()) {
            ...
            keymap.fastforward = SDL_GetKeyFromName(
                toml::find_or<std::string>(general, "fastforward", "Space").c_str());
            keymap.reset =
                SDL_GetKeyFromName(toml::find_or<std::string>(general, "reset", "F9").c_str());
            keymap.fullscreen = SDL_GetKeyFromName(
                toml::find_or<std::string>(general, "fullscreen", "F10").c_str());
        }
    }
    ...
    if (data.contains("gba")) {
        ...
        if (gba_result.is_ok()) {
            ...
            keymap.gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "a", "A").c_str())] =
                nba::InputDevice::Key::A;
            keymap.gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "b", "B").c_str())] =
                nba::InputDevice::Key::B;
            keymap.gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "l", "D").c_str())] =
                nba::InputDevice::Key::L;
            keymap.gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "r", "F").c_str())] =
                nba::InputDevice::Key::R;
            keymap.gba[SDL_GetKeyFromName(
                toml::find_or<std::string>(gba, "start", "Return").c_str())] =
                nba::InputDevice::Key::Start;
            keymap.gba[SDL_GetKeyFromName(
                toml::find_or<std::string>(gba, "select", "Backspace").c_str())] =
                nba::InputDevice::Key::Select;
            keymap.gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "up", "Up").c_str())] =
                nba::InputDevice::Key::Up;
            keymap
                .gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "down", "Down").c_str())] =
                nba::InputDevice::Key::Down;
            keymap
                .gba[SDL_GetKeyFromName(toml::find_or<std::string>(gba, "left", "Left").c_str())] =
                nba::InputDevice::Key::Left;
            keymap.gba[SDL_GetKeyFromName(
                toml::find_or<std::string>(gba, "right", "Right").c_str())] =
                nba::InputDevice::Key::Right;
        }
    }
}
...
void update_fullscreen() {
    SDL_SetWindowFullscreen(g_window,
                            g_config->video.fullscreen ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0);
}
